...
const regles = sansCommentaires.split('}');
const enDur = [];
for (const r of regles) {
  const m = r.match(/font-size:\s*(\d+)px/g);
  if (!m) continue;
  if (/material-symbols-outlined|dd-star|mac-imp/.test(r)) continue;   // des glyphes
  enDur.push(...m.map((t) => `${r.split('{')[0].trim().slice(0, 40)} → ${t}`));
}
assert.deepStrictEqual(enDur, [],
  'toute taille de texte du Point du jour doit passer par la charte (--taille-note / '
  + '--taille-texte / --taille-titre / --taille-grand) :\n  ' + enDur.join('\n  '));
