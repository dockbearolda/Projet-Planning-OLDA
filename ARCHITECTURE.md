...
| Agenda des retraits | Le planning rangé par JOUR : qui vient chercher quoi aujourd'hui, demain, après. Deux vues sur le même chargement — la liste du jour (heure, client, article, état) et le calendrier du mois (rien que les noms). Entrée en tête du rail, `#agenda` | Actif | `public/agenda.js`, `public/agenda.css`, `server.js` (`GET /api/agenda`) |
